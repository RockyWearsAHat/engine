...
- Server start reloads it. Every row still `running` on reload is marked `failed` (terminal, `alive:false`, `lost:true`, dedupe key released) regardless of PID. SARA sees status=failed with lost:true, re-dispatches. Nothing forgotten.
