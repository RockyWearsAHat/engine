...
declare global {
  interface Window {
    ...
    electronAPI?: {
      ...
      localServerHealthy?(): Promise<boolean>;
...
export const bridge = {
  ...
  async localServerHealthy(): Promise<boolean> {
    if (isTauri()) {
      return window.__TAURI__!.core.invoke<boolean>('local_server_healthy');
    }
    if (isElectron()) {
      return window.electronAPI!.localServerHealthy?.() ?? false;
    }
    return false;
  },
