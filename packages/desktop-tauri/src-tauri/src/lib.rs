...
mod tests {
    ...
    // ---- editor_preferences_from_config ------------------------------------
    ...
    fn apply_editor_preferences_normalizes_before_storing() {
        let mut cfg = AppConfig::default();
        // tab_size 3 should be normalized to 2
    ...
    fn project_path_uses_config_last_project_path_when_env_absent() {
        if env::var("PROJECT_PATH").is_ok() {
            return; // Skip if env var happens to be set in this environment
        }
        let mut cfg = AppConfig::default();
    ...
    fn project_path_falls_back_to_non_empty_path_when_no_config() {
        ...
        let cfg = AppConfig::default(); // last_project_path is None
        let result = project_path_for_server(&cfg);
        assert!(!result.is_empty(), "fallback path should be non-empty");
    }
}
